...
#This function checks for moving Selectors in the scene. It'll then update the shape key value
@persistent
def obj_init(scene):
...
class RenderSingleImageOperator(bpy.types.Operator):
    ...
    bl_description = """Renders a single 512px x 512px image for the shape key
    that you've selected. You must have a camera in your scene with the Category name in it"""
...
class RenderCategoryOperator(bpy.types.Operator):
    ...
    bl_description = """Renders all images for the category you've selected at 512px resolution.
    You must have a camera in your scene with the Category name in it"""
...
class RenderALLOperator(bpy.types.Operator):
    ...
    bl_label = "Render ALL"
    bl_description = """Renders ALL images for ALL categories for your model at 512px resolution.
    You must have cameras that match all of your Categories"""
...
def RenderAndSaveImage(folder, image_name):
    ...
    # Set the desired values for rendering
...
class SeparateLROperator(bpy.types.Operator):
    ...
    bl_description = """Separates the shape keys for the given category into Left and Right.
    DO THIS BEFORE YOU SET UP THE CATEGORY.
    Make sure you have two vertex groups LEFT SIDE and RIGHT SIDE with the respective parts of the model selected"""
...
class AddNeutralOperator(bpy.types.Operator):
    ...
    bl_description = """This will load the Neutral image for your character,
    as well as set up the header and selector icon for the category"""
...
class AddExpressionOperator(bpy.types.Operator):
    ...
    bl_description = """Add an expression for your character. This will add the expression
    image to your scene, add the driver to the shape keys, and set up all of the necessary values"""
...
class BulkAddExpressionsOperator(bpy.types.Operator):
    ...
    bl_description = """Bulk add expressions for the provided Category. If the image name
    matches the shape key name exactly, it will add that expression"""
...
def find_neutral_image(directory, category_name):
    ...
    Args:
        ...
        category_name: The name of the category that the image should be added to
              (this might need to change, because if your image doesn't match the Category name, it doesn't do anything)
